...
%---------更新于2017-10-06
disp(['第' num2str(counter_Folder-2) '个文件的供给矩阵'])
...
%0点要单独讨论
 ...
 save dataStore_hour_origin_XZ dataStore_hour
...
ds=datastore([Directory_Test '\' '20171009.csv'],...
    'VariableNames',VariableNames,'TextscanFormats',TextscanFormats,'IncludeSubfolders',false,'FileExtensions','.csv'...
    ,'Type','tabulartext');
...
% ------更新于2017-10-12--
% 考虑加入社区全部ZoneIdx变量，方便后续判断是否存在匹配的社区
LV_Output_ZoneIdx_all=LV_Output.ZoneIdx;
% ----------
...
%                 必须在此处先考虑一波社区内的所有节点相减
                if find(LV_Output_ZoneIdx_all==data_Test.ZoneIdx(counter_TestSize))%判断得出该车所在区域是否从属于某个社区
                    idx_belong_cmt=find(LV_Output_ZoneIdx_all==data_Test.ZoneIdx(counter_TestSize));
                    zoneidx_same_cmt=[];
                    for counter_belong_cmt=1:size(idx_belong_cmt,1)
                        idx_diff_cmt=find(LV_Output.Community==LV_Output.Community(idx_belong_cmt(counter_belong_cmt)));
                        zoneidx_same_cmt=[zoneidx_same_cmt;LV_Output.ZoneIdx(idx_diff_cmt)];
                    end
                    zoneidx_same_cmt=unique(zoneidx_same_cmt);
                    for counter_same_cmt=1:size(zoneidx_same_cmt,1)
                            Valid_Result{counter_hour,1}(1,zoneidx_same_cmt(counter_same_cmt))=...
                        Valid_Result{counter_hour,1}(1,zoneidx_same_cmt(counter_same_cmt))-1;
                    end
            ...
            if  str2double(tmp_reg{1})==0&&data_Test.ZoneIdx(counter_TestSize)~=0
               %                 必须在此处先考虑一波社区内的所有节点相减
                if find(LV_Output_ZoneIdx_all==data_Test.ZoneIdx(counter_TestSize))%判断得出该车所在区域是否从属于某个社区
                    ...
                    zoneidx_same_cmt=unique(zoneidx_same_cmt);
                    for counter_same_cmt=1:size(zoneidx_same_cmt,1)
                    Valid_Result{end,1}(1,zoneidx_same_cmt(counter_same_cmt))=...
                        Valid_Result{end,1}(1,zoneidx_same_cmt(counter_same_cmt))-1;
                    end
                else
